...
def train(
    ...
    log_freq = config.log_freq  # comment: change to inf for overfit test
    ...
    for i, (media_type, batch) in enumerate(iterator):
        # if i > 1:  # comment: for overfit test
        #     continue
...
def evaluate(
    ...
    val_scores = {}
    if (is_main_process() or config.gpu_num == 1) and len(save_preds) > 0:  # modified here
        if eval_name == 'scanqa':
            ...
            # [ASR] use calc_scanrefer_score_asr for evaluating on the ASR benchmark
...
def main(config):
    ...
    num_steps_per_epoch = sum(len(d) for d in train_loaders)  # comment: change here for overfit test
    config.scheduler.num_training_steps = num_steps_per_epoch * config.scheduler.epochs
    config.scheduler.num_warmup_steps = num_steps_per_epoch * config.scheduler.warmup_epochs
    torch.backends.cudnn.benchmark = True

    model_cls = eval(config.model.get('model_cls', 'Graph3DLLM'))
    (
        model,
        model_without_ddp,
        optimizer,
    ...
    if not config.evaluate:
        ...
        for epoch in range(start_epoch, config.scheduler.epochs):
            ...
            if is_main_process():
                ...
                # # QuatRoPE
                # if epoch == config.scheduler.epochs - 1:  # final epoch
                #     save_obj = {
                #         "model": state_dict,
                #         # "optimizer": optimizer.state_dict(),
                #         # "scheduler": scheduler.state_dict(),
                ...
                #         "global_step": global_step,
                #     }
                # comment: saving checkpoint here, close for overfit test
                
                # # QuatRoPE: split state_dict
                # state_dict_new = {}
                # for k in state_dict.keys():
                #     if 'llama_model.base_model.model.model.embed_tokens.weight' == k:
                #         # torch.save(state_dict[k], join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_embd.pth"))
                #         embd_obj = state_dict[k].detach().clone()
                #         print(f"embd_obj size: {embd_obj.numel() * embd_obj.element_size() / 1024 / 1024} MB")
                #         torch.save(embd_obj, join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_embd.pth"))
                #         # torch.save(state_dict[k][:64203], join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_embd_0.pth"))
                #         # torch.save(state_dict[k][64203:], join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_embd_1.pth"))
                #     elif 'llama_model.base_model.model.lm_head.weight' == k:
                #         lm_obj = state_dict[k].detach().clone()
                #         print(f"lm_obj size: {lm_obj.numel() * lm_obj.element_size() / 1024 / 1024} MB")
                #         torch.save(lm_obj, join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_lm.pth"))
                #         # torch.save(state_dict[k][:64203], join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_lm_0.pth"))
                #         # torch.save(state_dict[k][64203:], join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_lm_1.pth"))
                #     else:
                #         state_dict_new[k] = state_dict[k]
                # save_obj = {
                #     "model": state_dict_new,
                #     "config": config,
                #     "epoch": epoch,
                #     "global_step": global_step,
                # }
                ...
                #         return obj.numel() * obj.element_size()  # Tensor memory size
                #     else:
                #         return sys.getsizeof(obj)

                # size_bytes = estimate_size(save_obj)
                # print(f"Approximate size: {size_bytes / (1024 ** 2):.2f} MB")
                ...
                if ((epoch+1) % save_step_interval == 0 or epoch == config.scheduler.epochs - 1) and config.do_save and not config.debug:
                    if config.get("save_latest", False):
                        ...
                        torch.save(save_obj, join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}.pth"))
                        # torch.save(save_obj_A, join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_A.pth"))
                        # torch.save(save_obj_B, join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_B.pth"))
                        # torch.save(save_obj_C, join(config.output_dir, f"ckpt_{epoch:02d}_{global_step}_C.pth"))
